...
void ApparatusSpiceTarget::PlaceTarget(G4LogicalVolume* expHallLog)
{
  ...
  G4double placement = beampos*mm - target_Backer_thickness;
...
void ApparatusSpiceTarget::PlaceTargetBacker(G4LogicalVolume* expHallLog)
{
  G4cout << "Spice Target backer implemented " << G4endl;
  G4cout << "Back-Thick: " << target_Backer_thickness << G4endl;
  //NEED TO LINK BEAM POS TO HERE for move = three-vector for mosition
  G4ThreeVector move = G4ThreeVector(0.,0.,beampos*mm);//0 for now until read in 
  G4RotationMatrix* sRotate = new G4RotationMatrix;
...
void ApparatusSpiceTarget::PlaceTargetProtector(G4LogicalVolume* expHallLog)
{
...
//   beampos-(target_Backer_thickness/target_Backer_material_density)*2.0-(target_thickness/target_material_density)*2.0
  ...
  G4double placement = beampos*mm - (this->target_Backer_thickness) - (this->target_thickness);
