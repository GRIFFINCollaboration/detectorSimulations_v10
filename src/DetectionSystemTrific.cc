...
// 	G4Material* TrificCF4= new G4Material(name="TrificCF4",density, ncomponents=2);
	...
	// Dimensions 
	// ----------------------------
	this->fPCBThickness = 2.6*mm;
	this->fGridAngle = 30*deg;
	this->fSpacerZThickness = 1.0*cm;
...
// 	this->fWirePitch = 10*mm;//simpler for development
	...
	this->fChamberInnerD = 14.68*cm;
	this->fChamberOuterD = 15.91*cm;
	
	this->fPCBInnerD = 10.0*cm;
	this->fPCBOuterD = 14.0*cm; 
	this->fPCBCutDepth = 1.22*cm;
	this->fPCBCutHalfHeight = 1.5*cm;
	this->fRodD = 0.635*cm;
	this->fRodPlaceD = 12.8*cm;
	
	this->fChamberWindowZ = 5.2*cm;
	this->fWindowWasherZ = 1.5*cm;
	this->fWindowGridZ = this->fWindowWasherZ+0.8*cm;
// 	this->fWindowThickness = 6*um;
	this->fWindowThickness = setwindow;
	this->fWindowCoatingThickness = 0.9*um;
	this->fWindowInnerD = 7*cm;
	this->fWindowOuterD = 14.4*cm;
	this->fWindowChamberLength = 2.88*cm;
	this->fWindowPipeInnerD = 13*cm;

    //This is the correct offset to place the first grid at 53.4 cm which is correct for BAMBINO
	this->fTargetChamberZOffset = 53.4*cm-(fChamberWindowZ+fWindowGridZ);
	...
	this->fYgrid=2;
	this->fYwires={4,4,4,4,4,20};
    
	this->fXgrid=4;
	this->fXwires={3,3,3,3,3,10};
...
void DetectionSystemTrific::BuildPlaceYSense(G4LogicalVolume* TrificGasVol){
	...
	G4Material* material = G4Material::GetMaterial(this->fGasMaterial);
		if( !material ) {
		G4cout << " ----> Material " << this->fGasMaterial<< " not found, cannot build the Trific detector! " << G4endl;return;
	}
    ...
    for(unsigned int j=0;j<2;j++){
        ...
        //         
            ...
            // 	GasCellPartA->SetVisAttributes (G4VisAttributes::Invisible);   
                ...
                if(j%2)colour=!colour;
                if(colour)GasCellPartA->SetVisAttributes(vis_att);   
