...
G4double BeamDistribution::SelectDist(G4double RelProb){//the stopping distance for the beam is chosen here on an event by event basis using a random number generator to specify the probabilty
...
G4double BeamDistribution::SetSource(G4String SourceName, G4double EnIn, G4double rand){//rand saves loading an extra package
	
//   G4cout << SourceName << G4endl;
  G4double fEffEnergy = 0. ;
	if(SourceName == "Bismuth"){
// 		G4cout << "BISMUTH PEAK" << G4endl;
...
// 		G4cout << "OUTPUT " << fEffEnergy << G4endl;
		return fEffEnergy;
	} else if(SourceName == "Barium"){
		...
		G4double BaRand = rand*4.97185, BaEnSum;//adjusts for relatives
		G4int BaIter;
		...
		G4cout << "SOURCE NAME INVALID SO ENERGY SET TO PREVIOUS" << G4endl;
		return fEffEnergy = EnIn;
	}
	G4cout << "OUTPUT?? " << fEffEnergy << G4endl;
}