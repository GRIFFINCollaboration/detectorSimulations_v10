...
### Requirements
The main branch of detectorSimulations is confirmed to run on geant4.10.04 and ROOT 6.10.08. Do not use geant4.10.01 as it has a bug in the gamma de-excitation (see issue #35). For angular correlations there is a special branch (called angular-correlations) which runs with earlier geant4 versions.

The main (untested) change to the main branch compared to the angular-correlation branch is the removal of the explicit use of the obsolete G4FermiBreakup model in PhysListHadon. It has only been tested (so far) with a simulation of one million gamma rays of 1000 keV using only GRIFFIN (with 20 mm delrin). See also issue #40.
...
To do this, register on gitlab.com, have your account added to the GRIFFINCollaboration (i.e. send an email to Vinzenz with your gitlab login name so that he can add you), and register your ssh-keys with gitlab.
